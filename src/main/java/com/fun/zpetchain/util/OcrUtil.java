...
/**
 * tool class that handles the captcha and identifes the captcha
...
public class OcrUtil {
	...
	/**
	 * identify captcha by Tesseract via cmd
	 * you should set Tesseract environment first
	 * @author 2bears
	 * @since
	 * @param img
	 * @return
	 ...
	 * @return
	 */
	public String ocrByTess4j(BufferedImage img) {
		String result = "";
		ITesseract tess = new Tesseract();
		tess.setDatapath("F:\\Tesseract-OCR\\tessdata");
		tess.setLanguage("my");
    ...
    private static void setBorder(BufferedImage img) {
		...
		for(int y = 0; y < height; y++) {
			for(int x = 0; x < width; x++) {
	            if(x > borderXLeftUp && y > borderYLeftUp && 
	               x < width - borderXRightBottom && y < height - borderYRightBottom) {
	            	continue;
	            } else {
	            	img.setRGB(x, y, Color.WHITE.getRGB());
	            }	
		...
		try {
			...
			for(File f : folder.listFiles()) {
				if(f.isFile()) {
					...
					File ff = new File("F:\\testPic\\" + result + ".jpg");
				    ImageIO.write(img, "jpg", ff);	
